...
Text::Text() {
    ...
    _alloc=0;
    lines=reinterpret_cast<TextLine**>(calloc(0,sizeof(TextLine*)));
}
...
void Text::insert(size_t x,TextLine * line){
    if(x>=_alloc){
        ...
        lines=reinterpret_cast<TextLine**>(realloc(lines,sizeof(TextLine*)*(_alloc+1)));
...
void Text::grow(size_t to,size_t n,TextLine * p){
    if(to<=_alloc)return;
    lines=reinterpret_cast<TextLine**>(realloc(lines,sizeof(TextLine*)*to));
